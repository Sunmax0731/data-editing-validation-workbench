...
- 作業ディレクトリ: `D:\AI\VSCodeExtension\data-editing-validation-workbench`
...
```powershell
cd D:\AI\VSCodeExtension\data-editing-validation-workbench
npm test
npm start
```
...
5. VS Code extension + webview の UI または配布面を開き、検証結果をユーザーが次アクションに移せる粒度で確認する。
...
- docs ZIP が `dist/data-editing-validation-workbench-docs.zip` に存在する。
